...
class TestCursor:
    ...
    def test_streaming_changelog_cursor(
        ...
        max_wait_iterations = 60  # Do up to 60 iterations of fetchone() loop.
        wait_iterations = 0
        ...
        while cursor.may_have_results and wait_iterations < max_wait_iterations:
            # This is not actually a hard loop over fetchone() and attempted page fetching, since
            # the changelog fetching will buffer up multiple rows and return them one at a time on
            # each fetchone() call, as well as will pause
            # connection.statement_results_page_fetch_pause_millis between
            # attempts to fetch more results from the server when it runs out of buffered results,
            # which allows time for new results to be generated by the server.
            #
            # One never wants to sleep() in a fetchone() loop (far too many sleeps), since we
            # fetch whole pages of results at a time, but the changelog processor's internal
            # buffering isn't directly exposed via dbapi. So, the cursor implements internal
            # delay logic that only sleeps when it has exhausted its current buffer of results and
            # needs to fetch more from the server, which is the only time sleeping is appropriate.
            #
            # We may well write some additional methods on the cursor to better expose the number
            # of rows which are available in the internal buffer in the future (for fetchmany() use)
            # and perhaps a fetchsome() that returns however many rows are currently buffered (if
            # any).
            #
            # But for now, streaming changelog cursors are best consumed via either fetchone() or
            # iteration. Alas, fetchmany() is ill advised since it may end up blocking indefinitely
            # waiting for enough rows to fill the requested batch size, and with a streaming cursor
            # we'll never know if / when that batch size will be filled.
            #
