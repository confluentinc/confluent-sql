...
The driver stamps `Authorization: Bearer <bearer_token>` and `Confluent-Identity-Pool-Id` on
every Flink request. BYOIDC reaches the Flink data plane only -- Tableflow, Connectors, and the
...
conn = confluent_sql.connect(
    bearer_token=os.getenv("CONFLUENT_BEARER_TOKEN", ""),
