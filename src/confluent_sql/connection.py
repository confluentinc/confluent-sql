...
def connect(
    ...
    **kwargs: Any
    ...
    return Connection(
        flink_api_key, flink_api_secret, environment, compute_pool_id, organization_id, cloud_provider, cloud_region,
        api_key=api_key, api_secret=api_secret, host=host, **kwargs
    )
...
class Connection:
    ...
    def execute_statement(self, statement: str, parameters: Optional[Union[Tuple, List, Dict]] = None, statement_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.status_code not in [200, 201]:
                raise OperationalError(f"Failed to create statement: HTTP {response.status_code} - {response.text}")
    ...
    def get_statement_status(self, statement_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                raise OperationalError(f"Failed to get statement status: HTTP {response.status_code} - {response.text}")
    ...
    def get_statement_results(self, statement_name: str, page_token: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if response.status_code != 200:
                raise OperationalError(f"Failed to fetch results: HTTP {response.status_code} - {response.text}")
    ...
    def delete_statement(self, statement_name: str) -> None:
        ...
        try:
            ...
            if response.status_code != 204:
                raise OperationalError(f"Failed to delete statement: HTTP {response.status_code} - {response.text}")
