...
- Pushed the remaining `connect()`-only validation (`environment_id` and the endpoint-vs-`cloud_provider`/`cloud_region` requirement) down into `Connection.__init__()` as the single source of truth, so `connect()` and direct `Connection()` construction validate uniformly. Callers to `connect()` see no new errors. (#213)
