...
| `VARIANT`                                         | `dict` (OBJECT) / `list` (ARRAY) / typed scalar (`bool`, `int`, `float`, `Decimal`, `str`, `date`, `time`, `datetime`, `bytes`) / `UndecodableVariant` | _(n/a)_                       | Result decoding only. An OBJECT becomes a `dict`, an ARRAY a `list`, and each scalar its matching Python type (`bool`, `int`, `float`, `Decimal`, `str`, `date`, `time`, `datetime`, `bytes`). Nanosecond timestamps are truncated to microseconds (Python `datetime` limit). An undecodable node is returned as `UndecodableVariant`. Cannot be sent as a parameter, as Flink has no VARIANT literal syntax yet. |
