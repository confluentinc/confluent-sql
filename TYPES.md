...
| `TIMESTAMP`, `TIMESTAMP_WITHOUT_TIME_ZONE`        | `datetime.datetime` (naive)                              | `cast('2024-01-14 12:34:56.123456' as timestamp)`     | Provide timezone-lacking `datetime.datetime` instances.                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `ROW`                                             | `tuple`, `namedtuple`, `typing.NamedTuple`, `@dataclass` | `ROW(1, 'foo')`                                       | Parameter `ROW` values require `tuple`, `namedtuple`, `NamedTuple` or `@dataclass` instances, and are transcribed to literal `ROW` values positionally. Statement results containing `ROW` members will by default be mapped to auto-generated `collections.namedtuple` classes with named fields. Users may register their own `collections.namedtuple`, `typing.NamedTuple`, or `@dataclass` for use for `ROW` result mapping using the `connection.register_row_type()` method. |
| `VARIANT`                                         | `dict` (OBJECT) / `list` (ARRAY) / typed scalar (`bool`, `int`, `float`, `Decimal`, `str`, `date`, `time`, `datetime`, `bytes`) / `UndecodableVariant` | _(n/a)_                       | VARIANT values are encountered in results in two broad ways: from an explicit `PARSE_JSON()` (or similar) call within the statement itself, or when an underlying Kafka topic's serialization format maps onto Flink's VARIANT type — for example a JSON Schema carrying an open "{}" field. Result decoding only. A VARIANT toplevel OBJECT or member becomes a `dict`, an ARRAY a `list`, and each scalar its matching Python type (`bool`, `int`, `float`, `Decimal`, `str`, `date`, `time`, `datetime`, `bytes`). Nanosecond timestamps are truncated to microseconds (Python `datetime` limit). An undecodable node is returned as `UndecodableVariant`. Cannot be sent as a statement parameter, as Flink has no VARIANT literal syntax yet. |
...
## Code Samples Demonstrating Parameter Interpolation and Result Typing
